...
├── 📂 in/
│   ├── 📄 IncreaseTar10.sl4
│   ├── 📄 IncreaseTar10-WEL.har
│   ├── 📄 IncreaseTar50.sl4
│   ├── 📄 IncreaseTar50-WEL.har
│   ├── 📄 ...
├── 📂 map/
│   ├── 📄 OutputMapping.xlsx
├── 📂 out/
...
<details>
  ...
  <div class="note-box">
**Note**: <br>
(1) **Case-sensitive** — `"Exp1"` is different from `"exp1"`  
(2) **Display order matters** — files listed first will appear first in outputs and figures  
(3) **Matching names required** — simulations must use the same base name for both `.sl4` and `.har` files to align results correctly (e.g., `IncreaseTar10.sl4` and `IncreaseTar10.har`)
  </div>
</details>
...
- `"mil2bil"`: million → billion USD (/1000)

- `"bil2mil"`: billion → million USD (*1000) 

- `"pct2frac"`: percent → fraction (/100)  

- `"frac2pct"`: fraction → percent (*100)
...
<details>
  <summary class="toggle-summary">💻 AutoGTAP Codes</summary>
```{=html}
...
decimals         <- 2
rename_columns   <- TRUE       # Rename GTAP columns (e.g., "REG" → "Region")
